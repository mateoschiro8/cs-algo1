...
bool asegurarInt(int estimated, int expected){
    return estimated == expected;
}
...
bool test05_fibonacci(){
    ...
    bool res = asegurarInt(fibonacci(0), 0);
    res &= asegurarInt(fibonacci(1), 1);
    res &= asegurarInt(fibonacci(2), 1);
    res &= asegurarInt(fibonacci(3), 2);
    res &= asegurarInt(fibonacci(4), 3);
...
bool test06_maximoComunDivisor(){
    ...
    bool res = asegurarInt(maximoComunDivisor(1,1), 1);
    res &= asegurarInt(maximoComunDivisor(1,2), 1);
    res &= asegurarInt(maximoComunDivisor(6,2), 2);
    res &= asegurarInt(maximoComunDivisor(17,19), 1);
    res &= asegurarInt(maximoComunDivisor(100,0), 100);
    res &= asegurarInt(maximoComunDivisor(100,10), 10);
    res &= asegurarInt(maximoComunDivisor(28,56), 28);
    res &= asegurarInt(maximoComunDivisor(6,10), 2);
    res &= asegurarInt(maximoComunDivisor(-6,10), 2);
    res &= asegurarInt(maximoComunDivisor(10,-6), 2);
    res &= asegurarInt(maximoComunDivisor(0,1), 1);
...
bool test07_sumaDoble(){
    ...
    bool res = asegurarInt(sumaDoble(impares), 0);
    res &= asegurarInt(sumaDoble(pares), 2140);
    res &= asegurarInt(sumaDoble(mezcla), 1384);
    res &= asegurarInt(sumaDoble(vacio), 0);
    res &= asegurarInt(sumaDoble(paresNeg), 4);
    printEndTest(testNumber, testName);
    return res;
}
...
bool test08_cantPalabras(){
    ...
    res &= asegurarInt(cantPalabras("datos/loremIpsum.txt"), 98);
    res &= asegurarInt(cantPalabras("datos/losprofetas.txt"), 64);
